...
@task
def flake8(ctx):
    run("flake8 --max-line-length 100 am1bcc_charge")


@task
def update_manifest(ctx):
    """
    Updates manifest.json with the correct version of am1bcc_charge
    """
    spec = loads(open("manifest.json", "r").read())
    sys.path.append(os.path.dirname(__file__))
    import am1bcc_charge
    spec["version"] = am1bcc_charge.__version__
...
def docs(ctx):
    curdir = os.getcwd()
    run("cube_doc am1bcc_charge docs/source")
    run("floe_doc am1bcc_charge floes docs/source")
    os.chdir("docs")
    if sys.platform == "win32":
        run("make.bat html")
    else:
...
def clean(ctx):
    ...
    egg_path = "{}.egg-info".format("am1bcc_charge".replace("-", "_"))
